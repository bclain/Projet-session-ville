@section('content')
    <div class="container">
        ...
        <form action="#" method="post">
            @csrf
            @php
                $fields = $formulaireSoumis->data['fields'] ?? [];
            @endphp

            @foreach($fields as $field)
                <div class="form-group {{ $field['class'] ?? '' }}">
                    @if($field['type'] === 'checkbox')
                        <input type="checkbox" id="{{ $field['id'] }}" name="{{ $field['label'] }}" checked value="{{ $field['value'] }}" class="form-control" {{ $field['dg'] ? 'data-dg=true' : '' }} data-group="{{ $field['group'] ?? '' }}">
            ...
            <button type="submit" class="btn btn-primary">J'ai pris connaissance</button>
        </form>
    </div>
@endsection
